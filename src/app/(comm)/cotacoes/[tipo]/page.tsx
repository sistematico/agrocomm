...
import Layout from '@/layouts/main'
import { getCommodityPrices } from '@/lib/prices'
...
export default async function CotacoesPage({
  ...
  const { tipo } = await params
  const data = await getCommodityPrices(tipo)
    ...
    <Layout>
      <div className="space-y-6">
        <section className="bg-background p-6 rounded-lg border border-black/20">
          <h1 className="text-3xl font-bold mb-4">{tipo.toUpperCase()}</h1>
          {(data && lastUpdate) && (
            <>
              <p className="mb-4">
